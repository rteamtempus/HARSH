...
| Date | Summary | Ref |
|---|---|---|
| 2026-06-14 | Fix meeting "malformed JSON" failure (Release 0.13.1). `transcribe-meeting` + `extract-meeting` parsed Gemini output raw — but Gemini sometimes ```json-fences it, and `maxOutputTokens: 8192` truncated long transcripts mid-JSON. Added `stripCodeFences()` (mirrors the `llm` fn), raised caps (transcribe→65536, extract→32768), and a `finishReason !== STOP` truncation guard with actionable error text. Meeting page: "Retry transcription" button for failed-with-audio meetings (audio is preserved on failure). **Deployed both functions.** | (this commit) |
