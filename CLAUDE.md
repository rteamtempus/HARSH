...
| Date | Summary | Ref |
|---|---|---|
| 2026-06-13 | Fix display blank-screen on fresh devices: wall-mode (`base.css`) hard-codes light cream `--text-primary` for a dark canvas, but the app defaulted to light theme → invisible text on the light bg. Display now defaults to **dark** (set in `index.html` + applied in display `AppComponent` when no `harsh.theme` is stored). Theme system untouched; fix is display-app-scoped. Release 0.12.3. | (this commit) |
